...
function toggleRelatorioCheckboxes(source) {
  const items = document.querySelectorAll('.check-rel-item');
  items.forEach(cb => cb.checked = source.checked);
}

function gerarRelatorioCustomizado() {
  const selId = document.getElementById('alunoAntro')?.value;
  if (!selId) { showToast('Selecione um aluno primeiro', 'error'); return; }

  const incCadastro = document.getElementById('check-rel-cadastro').checked;
  const incAnamnese = document.getElementById('check-rel-anamnese').checked;
  const incAvaliacao = document.getElementById('check-rel-avaliacao').checked;

  if (!incCadastro && !incAnamnese && !incAvaliacao) {
    showToast('Selecione ao menos uma opção para o PDF', 'error');
    return;
  }
  ...
  let html = `
    <div style="font-family:sans-serif; color:#333; padding:20px; max-width:900px; margin:auto; border:1px solid #eee;">
      <div style="text-align:center; border-bottom:2px solid #0077ff; padding-bottom:1rem; margin-bottom:2rem;">
        <img src="logo.png" style="max-width:180px; margin-bottom:10px;" onerror="this.style.display='none'">
        <h1 style="color:#0077ff; margin:0;">TREINOFITASM</h1>
        <p style="margin:5px 0; font-weight:bold; font-size:1.2rem;">Relatório Personalizado de Avaliação</p>
        <p style="margin:5px 0; color:#666;">Adriano de Souza - Personal Trainer</p>
      </div>`;
  ...
  if (incCadastro) {
    html += `
      <section style="margin-bottom:2rem; background:#f8fafc; padding:15px; border-radius:8px;">
        <h3 style="border-bottom:2px solid #0077ff; padding-bottom:5px; margin-top:0; color:#1e3a8a;">1. Dados Cadastrais</h3>
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px;">
          <p><strong>Nome:</strong> ${aluno.nome}</p>
          <p><strong>Idade:</strong> ${aluno.idade} anos | <strong>Sexo:</strong> ${aluno.sexo}</p>
  ...
  if (incAnamnese) {
    html += `
      <section style="margin-bottom:2rem;">
        <h3 style="border-bottom:2px solid #0077ff; padding-bottom:5px; color:#1e3a8a;">2. Anamnese e Marcadores de Saúde</h3>
        ${lastAnam ? `
          <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px;">
            <p><strong>PA Repouso:</strong> ${lastAnam.paSistolica}/${lastAnam.paDiastolica} mmHg (${lastAnam.classPA})</p>
            <p><strong>FC Repouso:</strong> ${lastAnam.fcRepouso || '—'} bpm</p>
            <p><strong>Glicemia:</strong> ${lastAnam.glicemia || '—'} mg/dL</p>
            <p><strong>Colesterol:</strong> ${lastAnam.colesterol || '—'} mg/dL</p>
            <p style="grid-column:1/-1;"><strong>Patologias:</strong> ${lastAnam.doencas.join(', ') || 'Nenhuma'}</p>
            <p style="grid-column:1/-1;"><strong>Medicamentos:</strong> ${lastAnam.medicamentos || 'Nenhum'}</p>
          </div>
        ` : '<p>Nenhuma anamnese registrada.</p>'}
      </section>`;
  ...
  html += `
      <div style="text-align:center; margin-top:3rem; padding-top:2rem; border-top:1px solid #eee; font-size:0.9rem; color:#666;">
        <p>Relatório gerado em ${new Date().toLocaleDateString('pt-BR')}</p>
        <p style="font-weight:bold; color:#0077ff; margin-top:10px;">TREINOFITASM - Adriano de Souza</p>
      </div>
    </div>`;

  const win = window.open('', '_blank');
  win.document.write(`<html><head><title>Relatório Personalizado - ${aluno.nome}</title></head><body style="margin:0;">${html}</body></html>`);
  win.document.close();
  setTimeout(() => { win.print(); }, 500);
}
